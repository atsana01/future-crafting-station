...
const ProjectTabsBasic: React.FC<ProjectTabsProps> = ({ 
  activeProjectId, 
  onProjectChange, 
  children 
}) => {
  ...
  const fetchProjects = async () => {
    ...
    try {
      // Simple query to avoid type instantiation issues
      const projectsQuery = await (supabase as any)
        .from('projects')
      ...
      const { data, error } = projectsQuery;
      ...
      const projectList: Project[] = (data || []).map((item: any) => ({
        id: item.id,
        title: item.title,
      ...
      if (!activeProjectId && projectList.length > 0) {
        onProjectChange(projectList[0].id);
      }
    } catch (error) {
      console.error('Error fetching projects:', error);
    } finally {
...
export { ProjectTabsBasic as ProjectTabsFixed };